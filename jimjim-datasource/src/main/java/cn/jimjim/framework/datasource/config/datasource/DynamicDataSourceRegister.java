...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
     ...
     * 存储我们注册的数据源
     */
    private Map<String, DataSource> CUSTOM_DATA_SOURCES = new HashMap<String, DataSource>();
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        DynamicDataSourceContextHolder.dataSources.add("master");
        log.info("注册默认数据源成功");
        // 获取其他数据源配置
        ...
        for (Map map : configs) {
            ...
            CUSTOM_DATA_SOURCES.put(key, consumerDatasource);
        ...
        // 添加其他数据源
        mpv.add("targetDataSources", CUSTOM_DATA_SOURCES);
        // 将该bean注册为datasource，不使用springboot自动生成的datasource
        beanDefinitionRegistry.registerBeanDefinition("datasource", define);
        log.info("注册数据源成功，一共注册{}个数据源", CUSTOM_DATA_SOURCES.keySet().size() + 1);
    }
